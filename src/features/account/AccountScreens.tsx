...
export function MoreScreen() {
  ...
  return <Screen title={tx('More', 'المزيد')} subtitle={tx('Everything for your workday', 'كل ما تحتاجه في يومك')}>
    ...
    <Button title={tx('Switch demo workspace', 'تبديل المساحة التجريبية')} onPress={() => router.push('/demo-role')} /><Button title={tx('Leave demo', 'الخروج من النسخة التجريبية')} secondary onPress={() => router.replace('/login')} /><View style={{ alignItems: 'center', paddingTop: 12 }}><VoltexLogo dark={isDark} /></View><Label muted size={11}>{tx('VOLTEX · Promoter workspace · v1.0', 'فولتكس · مساحة المروّج · الإصدار 1.0')}</Label>
  </Screen>;
}
